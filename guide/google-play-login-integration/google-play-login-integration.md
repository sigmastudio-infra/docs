...
Android와 Web Application OAuth Client를 모두 생성한 뒤,  **Google Play Console**로 이동합니다.
...
**Resource Definition**에 현재 아무 값이 없는걸 볼수있는데 아래는 이 값을 추가하기 위한 방법입니다.
  
**Play 게임즈 서비스 - 구성** 페이지 (**사용자 성장 > Play 게임즈 서비스 > 설정 및 관리 > 구성**에서 **리소스 가져오기**를 클릭합니다.
...
Client ID의 경우 경로 표시(**Google Play Console >사용자 늘리기 > Play Games 서비스 >  설정 및 관리 > 설정**)
